...
// adapted to kha by Rafael Oliveira
...
class Gfx {
    ...
    public static function drawEllipse(g1:Graphics, xm:Int, ym:Int, a:Int, b:Int, col:Color) 
	{
        ...
        do {
            g1.setPixel( xm - x, ym + y, col );     //   I. Quadrant
            g1.setPixel( xm + x, ym + y, col );     //  II. Quadrant
            g1.setPixel( xm + x, ym - y, col );                 // III. Quadrant
            g1.setPixel( xm - x, ym - y, col );                 //  IV. Quadrant
            e2 = 2*err;
        ...
        while( y++ < b ) { // too early stop of flat ellipses a=1,
            g1.setPixel( xm, ym + y, col ); // -> finish tip of ellipse
            g1.setPixel( xm, ym - y, col );
        }
    }
    ...
    public static function drawEllipseRect(g1:Graphics,
										   x0:Int, y0:Int,
										   x1:Int, y1:Int, 
										   col:Color) {
        ...
        do {
            g1.setPixel( x1, y0, col ); //   I. Quadrant
            g1.setPixel( x0, y0, col ); //  II. Quadrant
            g1.setPixel( x0, y1, col ); // III. Quadrant
            g1.setPixel( x1, y1, col ); //  IV. Quadrant
            e2 = 2*err;
            if( e2 <= dy ){  // y step
                y0++;
                y1--;
        ...
        while( y0 - y1 <= b ) {                // too early stop of flat ellipses a=1
            g1.setPixel( x0 - 1, y0, col );                         // -> finish tip of ellipse
            g1.setPixel( x1 + 1, y0++, col );
            g1.setPixel( x0 - 1, y1, col );
            g1.setPixel( x1 + 1, y1--, col );
        }
    }
    ...
    public static function drawQuadBezier(g1:Graphics,
        ...
        if( x*( x2 - x1 ) > 0 ) {                                               // horizontal cut at P4?
            ...
            drawQuadBezierSeg( g1, x0, y0, x, Math.floor( r + 0.5 ), x, y, col );
        ...
        if( ( y0 - y1 )*( y2 - y1 ) > 0 ) {                                     // vertical cut at P6?
            ...
            drawQuadBezierSeg( g1, x0, y0, Math.floor( r + 0.5 ), y, x, y, col );
            r = ( x1 - x2 )*( t - y2 )/( y1 - y2 )+x2;                          // intersect P7 | P1 P2
            x0 = x;
            x1 = Math.floor( r + 0.5 );
            y0 = y1 = y;                                                        // P0 = P6, P1 = P7
        }
        drawQuadBezierSeg( g1, x0, y0, x1, y1, x2, y2, col );        // remaining part
    }

    // Fully ported, untested
    // draw a limited rational Bezier segment, squared weight
    public static function drawQuadRationalBezierSeg(g1:Graphics,
                                                     x0:Int,  y0:Int,
                                                     x1:Int,  y1:Int,
                                                     x2:Int,  y2:Int,
                                                     w:Float, col:Color) {
        ...
        } else { trace( 'failed to drawQuadRationalBeierSeg'); return; }
        ...
        if(cur != 0.0 && w > 0.0) {                  // no straight line
            ...
            if( w < 0.5 && ( dy > xy || dx < xy ) ) {    // flat ellipse, algorithm fails
                ...
                drawQuadRationalBezierSeg( g1, x0, y0, Std.int( dx ), Std.int( dy ), sx, sy, cur, col );// draw separately
                dx = Math.floor( ( w*x1 + x2 )*xy + 0.5 );
                dy = Math.floor( ( y1*w + y2 )*xy + 0.5 );
                drawQuadRationalBezierSeg( g1, sx, sy, Std.int( dx ), Std.int( dy ), x2, y2, cur, col );
                return;
            }
        ...
        drawLine( g1, x0, y0, x2, y2, col );                         // draw remaining needle to end
    }

    // Fully ported, untested
    // draw any quadratic rational Bezier curve
    public static function drawQuadRationalBezier(g1:Graphics,
                                                  x0:Int,  y0:Int,
                                                  x1:Int,  y1:Int,
                                                  x2:Int,  y2:Int,
                                                  w:Float, col:Color) {
        ...
        if( w >= 0.0 ){} else{ trace( 'Failed to drawQuadRationalBezier' ); return; }
        ...
        if( xx*( x2 - x1 ) > 0 ) {                             // horizontal cut at P4?
            ...
            drawQuadRationalBezierSeg( g1, x0, y0, x, Math.floor( yy + 0.5 ), x, y, ww, col );
        ...
        if( ( y0 - y1 )*( y2 - y1 ) > 0 ) {                                               // vertical cut at P6?
            ...
            drawQuadRationalBezierSeg( g1, x0, y0, Math.floor( xx + 0.5 ), y, x, y, ww, col );
        ...
        drawQuadRationalBezierSeg( g1, x0, y0, x1, y1, x2, y2, w*w, col );          // remaining
    }

    // Fully ported, untested
    // draw ellipse rotated by angle (radian)
    public static function drawRotatedEllipse(g1:Graphics,
                                              x:Int, y:Int,
                                              a:Int, b:Int,
        ...
        drawRotatedEllipseRect( g1, x - a, y - b, x + a, y + b, 4*zd*Math.cos( angle ), col );
    }

    // Fully ported, untested
    // rectangle enclosing the ellipse, integer rotation angle
    public static function drawRotatedEllipseRect(g1:Graphics,
                                                  x0:Int,   y0:Int,
                                                  x1:Int,   y1:Int,
                                                  zd:Float, col:Color) {
        var xd: Int = x1 - x0;
        ...
        if( zd == 0 ) return drawEllipseRect( g1, x0, y0, x1, y1, col );                     // looks nicer
        if( w != 0.0 ) w = ( w - zd )/( w + w );                                                        // squared weight of P1
        if( w <= 1.0 && w >= 0.0 ) {} else { trace( 'Failed to drawRotatedEllipseRect' ); return; }     // limit angle to |zd|<=xd*yd
        xd = Math.floor( xd*w + 0.5 );
        yd = Math.floor( yd*w + 0.5 );                                                                  // snap xe,ye to int
        drawQuadRationalBezierSeg( g1, x0, y0 + yd, x0, y0, x0 + xd, y0, 1.0 - w, col );
        drawQuadRationalBezierSeg( g1, x0, y0 + yd, x0, y1, x1 - xd, y1, w, col );
        drawQuadRationalBezierSeg( g1, x1, y1 - yd, x1, y1, x1 - xd, y1, 1.0 - w, col );
        drawQuadRationalBezierSeg( g1, x1, y1 - yd, x1, y0, x0 + xd, y0, w, col );
    }

    // Issue Porting  var *pxy;  and unknown exit command?  also &xy also overly complex for loop !
    // draw limited cubic Bezier segment
    public static function drawCubicBezierSeg(g1:Graphics,
                                              x0:Int,   y0:Int,
                                              x1:Float, y1:Float,
                                              x2:Float, y2:Float,
                                              x3:Int,   y3:Int,
        ...
        if( ( x1 - x0 )*( x2 - x3 ) < EP && (( x3 - x0 )*( x1 - x2 ) < EP || xb*xb < xa*xc + EP ) ){} else { trace( 'Failed to drawCubicBezierSeg' ); return; }
        if( ( y1 - y0 )*( y2 - y3 ) < EP && ( ( y3 - y0 )*( y1 - y2 ) < EP || yb*yb < ya*yc + EP ) ){} else { trace( 'Failed to drawCubicBezierSeg' ); return; }

        if( xa == 0 && ya == 0 ){                                       // quadratic Bezier
            sx = Math.floor( ( 3*x1 - x0 + 1 )/2 );
            sy = Math.floor( ( 3*y1 - y0 + 1)/2 );                      // new midpoint
            return drawQuadBezierSeg( g1, x0, y0, sx, sy, x3, y3, col );
        }
        ...
        drawLine( g1, x0, y0, x3, y3, col );       // remaining part in case of cusp or crunode
    }

    // Fully ported
    // draw any cubic Bezier curve
    public static function drawCubicBezier(g1:Graphics,
                                           x0:Int, y0:Int,
                                           x1:Int, y1:Int,
                                           x2:Int, y2:Int,
                                           x3:Int, y3:Int,
        ...
        for( i in 0...(n + 1) ){                 // draw each segment separately
            ...
            if( x0 != x3 || y0 != y3 ){                            // segment t1 - t2
                drawCubicBezierSeg( g1, x0,y0, x0 + fx1, y0 + fy1, x0 + fx2, y0 + fy2, x3, y3, col );
            }
            x0 = x3;
            y0 = y3;
            fx0 = fx3;
    ...
    public static function drawLineAA(g1:Graphics,
                                      x0:Int, y0:Int,
                                      x1:Int, y1:Int,
                                      col:Color) {
        var sx: Int = ( x0 < x1 )? 1 : -1;
        ...
        while( true ){
            ...
            setPixelAA( g1, x0, y0, Std.int( Math.abs( err - dx + dy ) ) >> 16, col );
            e2 = err;
            x2 = x0;
            if( 2*e2 >= -dx ){                                            // x step
                if( x0 == x1 ) break;
                var ed = Std.int( e2 + dy ) >> 16;
                if( (e2 + dy) < 0xff0000 ) setPixelAA( g1, x0, y0 + sy, ed, col );
                err -= dy;
                x0 += sx;
            }
            if( 2*e2 <= dy ) {                                             // y step
                ...
                if( dx - e2 < 0xff0000 ) setPixelAA( g1, x2 + sx, y0, ed, col );
                err += dx;
                y0 += sy;
                count++;
            }
    ...
    public static function drawCircleAA(g1:Graphics,
                                        xm:Int,  ym:Int,
                                        r:Float, col:Color) {
        var x: Int = Std.int( -r );
        var y: Int = 0;           // II. quadrant from bottom left to top right
        ...
        do {
            ...
            setPixelAA( g1, xm - x, ym + y, i, col );                             //   I. Quadrant
            setPixelAA( g1, xm - y, ym - x, i, col );                             //  II. Quadrant
            setPixelAA( g1, xm + x, ym - y, i, col );                             // III. Quadrant
            setPixelAA( g1, xm + y, ym + x, i, col );                             //  IV. Quadrant
            e2 = err;
            x2 = x;                                    // remember values
            if( err + y > 0 ){                                              // x step
                i = Std.int( 255*( err - 2*x - 1 )/r );                              // outward pixel
                if( i < 256 ) {
                    setPixelAA( g1, xm - x, ym + y + 1, i, col );
                    setPixelAA( g1, xm - y - 1, ym - x, i, col );
                    setPixelAA( g1, xm + x, ym - y - 1, i, col );
                    setPixelAA( g1, xm + y + 1, ym + x, i, col );
                }
                err += ++x*2 + 1;
            ...
            if( e2 + x2 <= 0 ) {                                             // y step
                ...
                if(i < 256 ){
                    setPixelAA( g1, xm - x2 - 1, ym + y, i, col );
                    setPixelAA( g1, xm - y, ym - x2 - 1, i, col );
                    setPixelAA( g1, xm + x2 + 1, ym - y, i, col );
                    setPixelAA( g1, xm + y, ym + x2 + 1, i, col );
                }
                err += ++y*2+1;
    ...
    public static function drawEllipseRectAA(g1:Graphics,
                                             x0:Int, y0:Int,
                                             x1:Int, y1:Int,
                                             col:Color) {
        var a: Int = Std.int(Math.abs( x1 - x0 ));
        ...
        if( a == 0 || b == 0 ) return drawLine( g1, x0, y0, x1, y1, col );
        ...
        while( true ){                             // approximate ed = sqrt( dx*dx + dy*dy )
            ...
            setPixelAA( g1, x0, y0, i, col );
            setPixelAA( g1, x0, y1, i, col );
            setPixelAA( g1, x1, y0, i, col );
            setPixelAA( g1, x1, y1, i, col );

            if( f = 2*err+dy >= 0 ){                  // x step, remember condition
                if( x0 >= x1 ) break;
                i = ed*( err + dx );
                if( i < 256 ) {
                    setPixelAA( g1, x0, y0 + 1, i, col );
                    setPixelAA( g1, x0, y1 - 1, i, col );
                    setPixelAA( g1, x1, y0 + 1, i, col );
                    setPixelAA( g1, x1, y1 - 1, i, col );
                }          // do error increment later since values are still needed
            }
            if( 2*err <= dx ){                                            // y step
                i = ed*(dy-err);
                if( i < 256 ){
                    setPixelAA( g1, x0 + 1, y0, i, col );
                    setPixelAA( g1, x1 - 1, y0, i, col );
                    setPixelAA( g1, x0 + 1, y1, i, col );
                    setPixelAA( g1, x1 - 1, y1, i, col );
                }
                y0++;
                y1--;
                err += dy += a;
    ...
    public static function drawQuadBezierSegAA(g1:Graphics,
                                               x0:Int, y0:Int,
                                               x1:Int, y1:Int,
                                               x2:Int, y2:Int,
                                               col:Color) {
        ...
        if( xx*sx <= 0 && yy*sy <= 0 ){} else { trace( 'Failed to drawQuadBezierSegAA' ); return; }      // sign of gradient must not change
        ...
        if( cur != 0 ){                                  // no straight line
            ...
            do {
                ...
                setPixelAA( g1, x0, y0, 255*Math.abs( err - dx - dy - xy )/ed, col );    // draw curve
                ...
                if( 2*err + dx > 0 ) { // x step
                    if( err - dy < ed ){
                        setPixelAA( g1, x0, y0 + sy, 255*Math.abs( err - dy )/ed, col );
                    }
                    x0 += sx;
                    dx -= xy;
                    err += dy += yy;
                ...
                if( y1 == 1 ){ // y step
                    if( cur < ed ) setPixelAA( g1, x1 + sx, y0, 255*Math.abs( cur )/ed, col );
                    y0 += sy;
                    dy -= xy;
                    err += dx += xx;
                }
        ...
        drawLineAA( g1, x0, y0, x2, y2, col ); // draw remaining needle to end
    }

    // draw an anti-aliased rational quadratic Bezier segment, squared weight
    public static function drawQuadRationalBezierSegAA(g1:Graphics,
                                                       x0:Int,  y0:Int,
                                                       x1:Int,  y1:Int,
                                                       x2:Int,  y2:Int,
                                                       w:Float, col:Color) {
        ...
        if( xx*sx <= 0.0 && yy*sy <= 0.0 ){} else { trace( 'Failed to drawQuadRationalBeizierSegAA'); return; };  // sign of gradient must not change
        ...
        if( cur != 0.0 && w > 0.0 ){                           // no straight line
            ...
            if( w < 0.5 && dy > dx ){              // flat ellipse, algorithm fails
                ...
                dx = Math.floor( ( w*x1 + x0 )*xy + 0.5 );
                dy = Math.floor( ( y1*w + y0 )*xy + 0.5 );
                drawQuadRationalBezierSegAA( g1, x0, y0, Std.int( dx ), Std.int( dy ), sx, sy, cur, col ); // draw apart
                dx = Math.floor( ( w*x1 + x2 )*xy + 0.5);
                dy = Math.floor( ( y1*w + y2 )*xy + 0.5);
                return drawQuadRationalBezierSegAA( g1, sx, sy, Std.int( dx ), Std.int( dy ), x2, y2, cur, col );
            }
            err = dx + dy - xy;                                       // error 1st step
            do {                                                      // pixel loop
                cur = Math.min( dx - xy, xy - dy );
                ...
                if( x1 < 256 ) setPixelAA( g1, x0, y0, x1, col );                   // draw curve
                if( f = 2*err + dy < 0 ){                                    // y step
                    if( y0 == y2 ) return;             // last pixel -> curve finished
                    if( dx - err < ed ){
                        setPixelAA( g1, x0 + sx, y0, 255*Math.abs( dx - err )/ed, col );
                    }
                }
                if( 2*err + dx > 0) {                                        // x step
                    if( x0 == x2 ) return;             // last pixel -> curve finished
                    if( err - dy < ed ) setPixelAA( g1, x0, y0 + sy, 255*Math.abs( err - dy )/ed, col );
                    x0 += sx;
                    dx += xy;
                    err += dy += yy;
                }
    ...
    public static function drawCubicBezierSegAA(g1:Graphics,
                                                x0:Int,   y0:Int,
                                                x1:Float, y1:Float,
                                                x2:Float, y2:Float,
                                                x3:Float, y3:Float,
        ...
        // check for curve restrains
        // slope P0-P1 == P2-P3     and  (P0-P3 == P1-P2      or  no slope change)
        if( ( x1 - x0 )*( x2 - x3 ) < EP && ( ( x3 - x0 )*( x1 - x2 ) < EP || xb*xb < xa*xc + EP ) ) {} else { trace('Failed to drawCubicBezierSegAA' ); return; }
        if( ( y1 - y0 )*( y2 - y3 ) < EP && ( ( y3 - y0 )*( y1 - y2 ) < EP || yb*yb < ya*yc + EP ) ) {} else { trace('Failed to drawCubicBezierSegAA' ); return; }

        if( xa == 0 && ya == 0 ) {                              // quadratic Bezier
            sx = Math.floor( ( 3*x1 - x0 + 1 )/2 );
            sy = Math.floor( ( 3*y1 - y0 + 1 )/2 );   // new midpoint
            return drawQuadBezierSegAA( g1, x0, y0, Std.int( sx ), Std.int( sy ), Std.int( x3 ), Std.int( y3 ), col );
        }
        x1 = ( x1 - x0 )*( x1 - x0 ) + ( y1 - y0 )*( y1 - y0 ) + 1;                     //line lengths
        x2 = ( x2 - x3 )*( x2 - x3 ) + ( y2 - y3 )*( y2 - y3 ) + 1;
        do {                                                // loop over both ends
            ...
            while( x0 != x3 && y0 != y3 ) {
                ...
                if( y1 < 256 ) setPixelAA( g1, x0, y0, y1, col );                  // draw curve
                ...
                if( 2*fy <= f ) {                           // x+ anti-aliasing pixel
                    if( py < ed ) setPixelAA( g1, Std.int( x0 + sx ), y0, 255*py/ed, col );      // draw curve
                    y0 += sy;
                    fy += f;                                      // y step
                }
                if( 2*fx <= f ) {                           // y+ anti-aliasing pixel
                    if( px < ed ) setPixelAA( g1, x0, Std.int( y2 + sy ), 255*px/ed, col );      // draw curve
                    x0 += sx; fx += f;                                      // x step
                }
            }
            //////  !!!????
            ...
            if( 2*ex < dy && 2*fy <= f + 2 ) {         // round x+ approximation pixel
                if( py < ed ) setPixelAA( g1, x0 + sx, y0, 255*py/ed, col );         // draw curve
                y0 += sy;
            }
        ...
        drawLineAA( g1, x0, y0, Std.int( x3 ), Std.int( y3 ), col );     // remaining part in case of cusp or crunode
    }

    // draw an anti-aliased line of width wd
    public static function drawLineWidth(g1:Graphics,
                                         x0:Int,   y0:Int,
                                         x1:Int,   y1:Int,
                                         wd:Float, col:Color) {
        var dx: Int = Std.int( Math.abs( x1 - x0 ) );
        ...
        while( true ) {                                    // pixel loop
            setPixelAA( g1, x0, y0, Std.int( Math.max( 0, 255*( Math.abs( err - dx + dy )/ed - wd + 1 )) ), col );
            e2 = err;
            x2 = x0;
            if( 2*e2 >= -dx ){ // x step
                e2 += dy;
                ...
                while( (e2 < ed*wd) && ((y1 != y2) || (dx > dy)) ){
                    setPixelAA( g1, x0, y2 += sy, Std.int( Math.max(0, 255 * (Math.abs(e2) / ed - wd + 1)) ), col );
                    e2 += dx;
                }
                if( x0 == x1 ) break;
                e2 = err;
            ...
            if( 2*e2 <= dy ){ // y step
                ...
                while( ( e2 < ed*wd ) && ( (x1 != x2) || (dx < dy)) ){
                    setPixelAA( g1, x2 += sx, y0, Std.int( Math.max(0, 255 * (Math.abs(e2) / ed - wd + 1)) ) , col );
                    e2 += dy;
                }
                if( y0 == y1 ) break;
                err += dx;
...
//
    public static function drawQuadSpline(g1:Graphics,
                                          n:Int,
                                          x:Array<Int>,
										  y:Array<Int>,
                                          col:Color) {                         // draw quadratic spline, destroys input arrays x,y
        var M_MAX: Int = 6;
        var mi: Float = 1;
        ...
        if( n > 1 ) {} else { trace( 'Failed to drawQuadSpline' ); return; }                        // need at least 3 points P[0]..P[n]
        ...
        for( i in 0...n-2 ){ //= n-2; i > 0; i--) {                           // back substitution
            ...
            drawQuadBezier( g1, Std.int(( x0 + x1 )/2), Std.int(( y0 + y1 )/2), x1, y1, x2, y2, col );
            x2 = Std.int( ( x0 + x1 )/2 );
            x1 = x0;
            y2 = Std.int( ( y0 + y1 )/2 );
            y1 = y0;
        }
        drawQuadBezier( g1, x[ 0 ], y[ 0 ], x1, y1, x2, y2, col );
    }

    // draw cubic spline, destroys input arrays x,y
    public static function drawCubicSpline(g1:Graphics,
                                           n:Int,
                                           x:Array<Int>,
										   y:Array<Int>,
                                           col:Color) {
        ...
        var y2: Int;
        // !!!!!
        if( n > 2 ){} else { trace( 'Failed to drawCubicSpline' ); return; }                        // need at least 4 points P[0]..P[n]
        ...
        drawCubicBezier( g1, x3, y3, Std.int( ( x2 + x4 )/2 ), Std.int( ( y2 + y4 )/2 ), x4, y4, x4, y4, col );
        ...
        drawCubicBezier(    g1, x4, y4
                        ...
                        ,   Math.floor( ( x1 + 2*x2 )/3 + 0.5), Math.floor( ( y1 + 2*y2 )/3 + 0.5 )
                        ,   x3, y3, col );
        drawCubicBezier( g1, x0, y0, x0, y0, Std.int( ( x0 + x1 )/2 ), Std.int( ( y0 + y1 )/2 ), x4, y4, col );
    }
